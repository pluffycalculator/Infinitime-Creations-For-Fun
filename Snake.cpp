...
void Snake::createStartScreen() {
    ...
    lv_label_set_text(label, "Start Snake Game");
    lv_obj_align(label, nullptr, LV_ALIGN_CENTER, 0, -30); // Center the label

    // Create a button for starting the game
    lv_obj_t* startBtn = lv_btn_create(lv_scr_act(), nullptr);
    startBtn->user_data = this;
    lv_obj_set_event_cb(startBtn, btnEventHandler);
    lv_obj_set_size(startBtn, 150, 50);
    lv_obj_align(startBtn, nullptr, LV_ALIGN_CENTER, 0, 30); // Center the button
...
void Snake::startGame() {
    ...
    for (int i = 0; i < 5; ++i) {
        snakePoints.push_back({125, 125}); // Start at the center (125, 125)
    }
    ...
    lv_style_set_radius(&foodStyle, LV_STATE_DEFAULT, 5); // Set food as a circle

    food = lv_obj_create(lv_scr_act(), nullptr);
    lv_obj_add_style(food, LV_OBJ_PART_MAIN, &foodStyle);
    lv_obj_set_size(food, 10, 10); // Set food size
...
void Snake::moveSnake() {
 ...
 // Check for collision with food
  if (checkCollision(headX, headY)) {
    ...
    currentInterval = std::max(200, currentInterval - 15);
